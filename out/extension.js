...
const openEncryptedDocs = new Map();
const rememberedPasswords = new Map();
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.selectDirectory', async () => {
    ...
    // Comando: Refrescar
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.refresh', () => {
        notesProvider.refresh();
    }));
    // Comando: Crear archivo
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.createNote', async (node) => {
        const notesDir = context.globalState.get('markdownNotes.notesDirectory');
        if (!notesDir) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.createFolder', async (node) => {
        const notesDir = context.globalState.get('markdownNotes.notesDirectory');
        if (!notesDir) {
            vscode.window.showWarningMessage('Por favor, selecciona un directorio de notas primero');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.openNote', async (node) => {
        ...
        if ((0, encryption_1.isEncrypted)(content)) {
            if (!unlockedNotes.has(filePath)) {
                ...
                const decrypted = (0, encryption_1.decrypt)(content, password);
                if (!decrypted) {
    ...
    // Comando: Renombrar
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.renameNote', async (node) => {
        if (!node.resourceUri) {
            return;
        }
    ...
    // Comando: Eliminar
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.deleteNote', async (node) => {
        if (!node.resourceUri) {
            return;
        }
        ...
        const isFolder = node.contextValue === 'folder';
        const confirmation = await vscode.window.showWarningMessage(`¿Estás seguro de que quieres eliminar ${isFolder ? 'la carpeta' : 'la nota'} "${itemName}"?`, { modal: true }, 'Eliminar');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.protectNote', async (node) => {
        if (!node.resourceUri) {
            return;
        }
        const filePath = node.resourceUri.fsPath;
        ...
        const encrypted = (0, encryption_1.encrypt)(content, password);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.unlockNote', async (node) => {
        if (!node.resourceUri) {
            return;
        }
        const filePath = node.resourceUri.fsPath;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.lockNote', async (node) => {
        if (!node.resourceUri) {
            return;
        }
        const filePath = node.resourceUri.fsPath;
    ...
    // Comando: Mover nota
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.moveNote', async (node) => {
        if (!node.resourceUri) {
            return;
        }
        const notesDir = context.globalState.get('markdownNotes.notesDirectory');
        ...
        const selectedFolder = await vscode.window.showQuickPick(['📁 Raíz', ...folders.map(f => `📂 ${path.relative(notesDir, f)}`)], { placeHolder: 'Selecciona la carpeta destino' });
        if (!selectedFolder) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.pasteContent', async () => {
        const notesDir = context.globalState.get('markdownNotes.notesDirectory');
        if (!notesDir) {
            vscode.window.showWarningMessage('Por favor, selecciona un directorio de notas primero');
            return;
    ...
    const savedDir = context.globalState.get('markdownNotes.notesDirectory');
    if (savedDir) {
        notesProvider.refresh();
    }
    ...
    const wikiLinkDefinitionProvider = vscode.languages.registerDefinitionProvider(['markdown', 'plaintext'], new WikiLinkDefinitionProvider(context));
    // Wiki Links - Hover Provider
    const wikiLinkHoverProvider = vscode.languages.registerHoverProvider(['markdown', 'plaintext'], new WikiLinkHoverProvider(context));
    // Wiki Links - Document Link Provider para cmd+click
    const wikiLinkDocumentLinkProvider = vscode.languages.registerDocumentLinkProvider(['markdown', 'plaintext'], new WikiLinkDocumentLinkProvider());
    // Comando: Navegar a wiki link
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.followWikiLink', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.navigateToWikiLinkFromClick', async (args) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.showGraphView', async () => {
        const notesDir = context.globalState.get('markdownNotes.notesDirectory');
        if (!notesDir) {
            vscode.window.showWarningMessage('Por favor, selecciona un directorio de notas primero');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.copyItem', async (node) => {
        if (!node?.resourceUri)
            return;
        const itemPath = node.resourceUri.fsPath;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.duplicateItem', async (node) => {
        if (!node?.resourceUri)
            return;
        const itemPath = node.resourceUri.fsPath;
        const isFolder = node.contextValue === 'folder';
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.revealInFileExplorer', async (node) => {
        if (!node?.resourceUri) {
    ...
    // Comando: Pegar
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.pasteItem', async (node) => {
        const copiedItem = context.globalState.get('markdownNotes.copiedItem');
        if (!copiedItem) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('markdownNotes.configureDirectory', async () => {
        const folderUri = await vscode.window.showOpenDialog({
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
    ...
    const wikiLinkCompletionProvider = vscode.languages.registerCompletionItemProvider('markdown', new WikiLinkCompletionProvider(context), '[' // Trigger character
    );
...
async function handleImageFromClipboard(imageBuffer, notesDir, notesProvider) {
    const options = [
...
async function saveImageBufferToAssets(imageBuffer, notesDir) {
...
async function createNoteWithImageBuffer(imageBuffer, notesDir, notesProvider) {
...
async function getFilesFromClipboard() {
    return new Promise((resolve) => {
        const { exec } = require('child_process');
        const os = require('os');
        const platform = os.platform();
...
async function handleFilesFromClipboard(filePaths, notesDir, notesProvider) {
...
async function copyImageToAssets(imagePath, notesDir) {
    const assetsDir = path.join(notesDir, 'assets');
    if (!fs.existsSync(assetsDir)) {
        fs.mkdirSync(assetsDir, { recursive: true });
    }
...
async function createNoteWithLocalImage(imagePath, notesDir, notesProvider) {
...
async function createMarkdownNoteFromFile(content, originalFileName, notesDir, notesProvider) {
...
async function createCustomExtensionFileFromContent(content, notesDir, notesProvider) {
...
async function handlePasteContent(content, notesDir, notesProvider) {
...
async function handleImagePaste(content, notesDir, notesProvider) {
    const options = [
        '📁 Guardar en carpeta assets',
        '📄 Crear nueva nota con imagen'
    ];
...
async function handleImageUrlPaste(content, notesDir, notesProvider) {
    const options = [
...
async function handleTextPaste(content, notesDir, notesProvider) {
    const options = [
        '📝 Crear como nota Markdown (.md)',
...
async function saveImageToAssets(base64Content, notesDir) {
    const assetsDir = path.join(notesDir, 'assets');
    if (!fs.existsSync(assetsDir)) {
        fs.mkdirSync(assetsDir, { recursive: true });
    }
...
async function createNoteWithImage(base64Content, notesDir, notesProvider) {
    const fileName = await vscode.window.showInputBox({
        prompt: 'Nombre de la nota (sin extensión)',
        placeHolder: 'nota-con-imagen'
    });
...
async function downloadImageToAssets(imageUrl, notesDir) {
    const assetsDir = path.join(notesDir, 'assets');
    if (!fs.existsSync(assetsDir)) {
        fs.mkdirSync(assetsDir, { recursive: true });
    }
...
async function createNoteWithImageUrl(imageUrl, notesDir, notesProvider) {
    const fileName = await vscode.window.showInputBox({
        prompt: 'Nombre de la nota (sin extensión)',
        placeHolder: 'nota-con-imagen'
    });
...
// Crear nota markdown
async function createMarkdownNote(content, notesDir, notesProvider) {
    const fileName = await vscode.window.showInputBox({
        prompt: 'Nombre de la nota (sin extensión)',
        placeHolder: 'nueva-nota'
    });
...
async function createCustomExtensionFile(content, notesDir, notesProvider) {
    const extension = await vscode.window.showInputBox({
        prompt: 'Extensión del archivo (sin punto)',
        placeHolder: 'txt',
        validateInput: (value) => {
...
class NotesProvider {
    constructor(context) {
        ...
        this.dropMimeTypes = ['application/vnd.code.tree.markdownNotesExplorer', 'text/uri-list'];
    ...
    async handleDrop(target, dataTransfer) {
        const notesDir = this.context.globalState.get('markdownNotes.notesDirectory');
        if (!notesDir) {
            return;
        }
        let targetDir;
    ...
    async getChildren(element) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                items.push(new NoteItem(entry.name, vscode.Uri.file(fullPath), vscode.TreeItemCollapsibleState.Collapsed, 'folder'));
            }
            else {
                ...
                if (isMarkdown) {
                    ...
                    encrypted = (0, encryption_1.isEncrypted)(content);
                    unlocked = unlockedNotes.has(fullPath);
                    isProtected = rememberedPasswords.has(fullPath);
                }
                items.push(new NoteItem(entry.name, vscode.Uri.file(fullPath), vscode.TreeItemCollapsibleState.None, 'file', encrypted || isProtected, unlocked, isMarkdown));
            }
        }
        items.sort((a, b) => {
...
class NoteItem extends vscode.TreeItem {
    constructor(label, resourceUri, collapsibleState, contextValue, isEncrypted = false, isUnlocked = false, isMarkdown = true) {
...
function extractWikiLinkAtPosition(line, characterPosition) {
    const regex = /\[\[([^\]]+)\]\]/g;
    let match;
...
function resolveWikiLinkPath(linkText, notesDir) {
    // Normalizar espacios
...
async function navigateToWikiLink(linkText, context, notesProvider) {
    const notesDir = context.globalState.get('markdownNotes.notesDirectory');
    if (!notesDir) {
        vscode.window.showWarningMessage('Por favor, selecciona un directorio de notas primero');
        return;
...
async function navigateToWikiLinkRelative(linkText, sourceUri, context, notesProvider) {
    const notesDir = context.globalState.get('markdownNotes.notesDirectory');
    if (!notesDir) {
        vscode.window.showWarningMessage('Por favor, selecciona un directorio de notas primero');
        return;
...
async function createFileForWikiLink(filePath, _linkText) {
...
class GraphViewPanel {
    static createOrShow(extensionUri, notesDir) {
        ...
        // Crear nuevo panel
        const panel = vscode.window.createWebviewPanel('markdownGraphView', 'Vista de Grafo', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _createFileForPath(filePath) {
        const dir = path.dirname(filePath);
        // Crear directorio si no existe
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true });
    ...
    _resolveWikiLinkPath(linkText, notesDir) {
        // Normalizar espacios
        const normalizedLink = linkText.trim().replace(/\s+/g, ' ');
    ...
    _findFileInDirectory(dir, fileName) {
        if (!fs.existsSync(dir))
            return null;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
...
class WikiLinkCompletionProvider {
            ...
            return this._getAllFiles(notesDir).map(fileInfo => {
                ...
                if (beforeCursor.includes('[[')) {
                    ...
                    const endIndex = afterCursor.indexOf(']]') !== -1 ?
                        ...
                        position.character;
                    item.range = new vscode.Range(position.line, startIndex, position.line, endIndex);
                    item.insertText = fileInfo.insertText;
                }
                else {
    ...
    _getAllFiles(dir) {
        const files = [];
        const scanDirectory = (currentDir, _basePath = '') => {
            try {
                const entries = fs.readdirSync(currentDir, { withFileTypes: true });
                for (const entry of entries) {
                    const fullPath = path.join(currentDir, entry.name);
